...
const generateContent = (task: UploadTask) => {
...
const generateCompleted = (task: UploadTask) => {
...
export const uploadTasksSlice = createSlice({
  ...
  reducers: {
    ...
    completedCountPlusOne(state, action) {
      ...
      if (taskIndex !== -1) {
        const task: UploadTask = state.tasks[taskIndex];
    ...
    updateTask(state, action) {
      ...
      if (taskIndex !== -1) {
        updatedTask.isCompleted = generateCompleted(updatedTask);
        updatedTask.content = generateContent(updatedTask);
