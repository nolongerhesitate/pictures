...
import { UploadTask, ErrorDetail } from "@/app/lib/types";

export default function TaskItem({ task }: { task: UploadTask }) {
  const reduxDispatch = useDispatch();
  const { onOpen, onClose, isOpen } = useDisclosure();
  const allUploadTasks = useSelector(selectUploadTasks);
  const curErrorDetails: ErrorDetail[] = allUploadTasks.errorDetails.filter((item: ErrorDetail) => item.taskId === task.id);
  const processingVal: number = Math.round(task.completedCount / task.totalCount * 100);
