...
import { DataResult, DataResultStatus, Picture, SetUseState } from "@/app/lib/types";
...
export default function MainContent({
  ...
  const [isDownloadSelectedPics, setIsDownloadSelectedPics] = useState<boolean>(false);
  ...
  const getPictures = async () => {
    try {
      setIsLoading(true)
      const result: DataResult<Picture[]> = await apiUtil.getAllPictures(pictureState.searchFeed, currentPage, false);
  ...
  const downloadSelectedPics = async () => {
    try {
      selectedPicIndices.forEach(async index => {
        ...
        a.href = url;
        a.download = picture?.displayName;
